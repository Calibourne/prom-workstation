...
def sidebar_filter(log, column_map):
    ...
    # Identify mandatory and optional columns
    ...
    with st.sidebar:
        # ✅ Fix: `expander` now correctly renders inside the sidebar
        with st.expander("Column Selection"):
            selected_columns = st.multiselect("Select Columns to Display:", additional_columns, default=[resource_col if resource_col else None])
        ...
        start_event = st.selectbox("Select Start Event:", ["All"] + available_activities)
        end_event = st.selectbox("Select End Event:", ["All"] + available_activities)

        # ✅ Resource & Activity Filtering
        ...
        with st.expander("Activity Filter"):
